...
type Props = NativeStackScreenProps<RootStackParamList, "game-comment">;
...
const CommentList: React.FC<Props> = ({ navigation, route }) => {
  ...
  const { loading, run } = useRequest(
    httpRequest.fetchGameComment.bind(httpRequest),
    {
      defaultParams: [{ gameId, page: 1 }],
      onSuccess(data) {
  ...
  return (
    <BgBox style={styles.container}>
      <View style={styles.full}>
        ...
        />
        <SendComment gameId={gameId} refresh={initRefresh} />
      </View>
    </BgBox>
  );
};
